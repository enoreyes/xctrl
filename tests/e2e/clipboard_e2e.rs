...
#[test]
fn e2e_clipboard_set_get_roundtrip() {
    if should_skip() {
        return;
    }
...
#[test]
fn e2e_clipboard_set_get_json_roundtrip() {
    if should_skip() {
        return;
    }
    let _lock = SystemLock::acquire("clipboard");
...
fn e2e_clipboard_clear() {
    ...
    // Get after clear — should be empty or report no content
    let output = run_cmd(&["clipboard", "get"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    // After clearing, clipboard should be empty — either empty stdout or empty text
    // Some platforms may return an error for empty clipboard, which is acceptable
...
#[test]
fn e2e_clipboard_special_characters() {
    if should_skip() {
        return;
    }
    let _lock = SystemLock::acquire("clipboard");
